...
const handleChat = async (req, res) => {
  try {
    ...
    const response = await geminiService.chatWithFinancialAdvisor(
      message,
      context,
    );
...
const getSmartSuggestions = async (req, res) => {
  try {
    ...
    const expenses = await Expense.find({ userId: req.user.id })
      .limit(20)
      .sort({ date: -1 });
    const suggestions = await geminiService.suggestExpenseCompletion(
      partialInput,
      expenses,
    );
...
const getSmartBudgets = async (req, res) => {
  try {
    ...
    const budgetSuggestions =
      await geminiService.generateSmartBudgets(expenses);
...
const parseVoice = async (req, res) => {
  try {
    const text = req.body?.text || req.body?.voiceText || "";
    const parsedData = await geminiService.parseVoiceExpense(text);

    if (!parsedData && text) {
      const localParsed = geminiService.parseExpenseFromMessage(text);
      if (localParsed) {
        return res.json({
          ...localParsed,
          confidence: 0.5,
