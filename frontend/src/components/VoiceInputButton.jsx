...
const VoiceInputButton = ({ onParsedData, pageContext = "general" }) => {
  ...
  const handleTranscript = async (text) => {
    try {
      ...
      if (response.data) {
        ...
        toast.error(
          "Couldn't understand the details. Try saying 'Spent 500 on coffee'.",
        );
      }
    } catch (error) {
  ...
  return (
    <button
      ...
      className={`relative p-3 rounded-full transition-all duration-300 transform hover:scale-110 active:scale-95 shadow-md flex items-center gap-2 ${
        isListening
