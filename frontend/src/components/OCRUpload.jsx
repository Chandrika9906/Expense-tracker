...
const OCRUpload = ({ onExpenseExtracted, onClose }) => {
  ...
  const getConfidenceColor = (confidence) => {
    ...
    if (safeConfidence >= 0.8) return "text-green-600 bg-green-100";
    if (safeConfidence >= 0.6) return "text-yellow-600 bg-yellow-100";
          ...
          {extractedData && (
            <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                {(() => {
                  const confidenceValue = Number(extractedData.confidence) || 0;
                  return (
                    <div
                      className={`px-3 py-1 rounded-full text-xs font-medium ${getConfidenceColor(extractedData.confidence)}`}
                    >
