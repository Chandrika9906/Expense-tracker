...
const Bills = () => {
  ...
  const handlePredictAmount = async () => {
    ...
    try {
      ...
      const history = selectedBill
        ? await billService.getBillHistory(selectedBill._id)
        : { data: [] };
      ...
      if (response.data.prediction > 0) {
        setFormData({
          ...formData,
          amount: response.data.prediction.toString(),
        });
        toast.success(`Predicted amount: ₹${response.data.prediction}`, {
          icon: "🤖",
        });
      } else {
    ...
    switch (activeFilter) {
      case "upcoming":
        return bills.filter((bill) => {
          ...
          return (
            !bill.isPaid && dueDate >= today && dueDate <= sevenDaysFromNow
          );
        });
      case "paid":
      ...
      case "overdue":
        return bills.filter(
          (bill) => !bill.isPaid && new Date(bill.dueDate) < today,
        );
      default:
        return bills;
    }
  ...
  const stats = {
    ...
    upcoming: bills.filter((b) => {
      ...
      return (
        ...
        new Date(b.dueDate) <= sevenDays
      );
    }).length,
    overdue: bills.filter((b) => !b.isPaid && new Date(b.dueDate) < new Date())
      .length,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-red-600 p-6 rounded-xl shadow-lg text-white">
          <div className="flex items-center justify-between">
            <div>
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="flex flex-wrap gap-2">
          {["all", "upcoming", "overdue", "paid", "unpaid", "recurring"].map(
            (filter) => (
              <button
                key={filter}
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        autoCreateExpense: e.target.checked,
                      })
                    }
                    className="w-5 h-5 text-primary rounded focus:ring-2 focus:ring-primary"
                  />
              ...
              <div className="mb-6 p-4 bg-primary/10 rounded-xl">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  ...
                  {selectedBill?.isRecurring
                    ? `Recurring (${selectedBill?.recurringInterval})`
                    : "One-time"}
                </p>
              </div>
